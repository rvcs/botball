...
int main()
{
	...
	write_byte(angle1>>8);
	write_byte(angle1&0xff);//turns right from start
	...
	write_byte(distance1&0xff);//--------------------move to first pom location
	...
	write_byte(distance3&0xff);	//------------------------go to second pom group
	...
	write_byte(distance2&0xff);	//-------------------ends up in end zone
